      ...
      for (int i = il; i <= ir; i++){
        ...
        for (int j = jl; j <= jr; j++){
          ...
          for (int k = kl; k <= kr; k++){
            ...
            // XXX for 3d
            double phippdux = phipp * (up[ii][jj][kk][X] - (vp[p][X] + (omegap[p][Y]*rz - omegap[p][Z]*ry))); 
            double phippduy = phipp * (up[ii][jj][kk][Y] - (vp[p][Y] + (omegap[p][Z]*rx - omegap[p][X]*rz))); 
            double phippduz = phipp * (up[ii][jj][kk][Z] - (vp[p][Z] + (omegap[p][X]*ry - omegap[p][Y]*rx))); 
            sendbuf[0] += phippdux;
            sendbuf[1] += phippduy;
            sendbuf[2] += phippduz;
