...
d = np.loadtxt("dat/particles.dat")[::,:]
fig,axes = plt.subplots(3,2,layout="constrained")
axes = axes.ravel()
axes[0].plot(d[:,1],d[:,2],"-")
axes[1].plot(d[:,1],d[:,3],"r:")
axes[2].plot(d[:,1],d[:,4],"b-.")
axes[3].plot(d[:,1],d[:,5],"-")
axes[4].plot(d[:,1],d[:,6],"r:")
axes[5].plot(d[:,1],d[:,7],"b-.")
# plt.xlabel(r"$t$")
# plt.ylabel(r"$x,v,f$")
plt.savefig("v.png")
plt.close()
...
def func(frame):
  ...
  d = np.vstack(
    [
      np.loadtxt(f"dat/fluid_{1000*(frame+1)}_{k}.dat",skiprows=0) for k in range(procs)
    ]
  ).reshape(NX,NY,5)
