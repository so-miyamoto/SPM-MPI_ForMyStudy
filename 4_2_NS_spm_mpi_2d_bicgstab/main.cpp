...
main(int argc, char** argv)
{
  ...
  const double F_EX[DIM] = {0.01,0.0}; // body force
  ...
  + F_EX[Y]
    ...
    for(int i = 1; i <= NXloc; i++){ 
      for(int j = 1; j <= NY; j++){
        ...
        cgb[i][j] = (0.5/DT) * (u_tmp[i+1][j][X] - u_tmp[i-1][j][X] + u_tmp[i][j+1][Y] - u_tmp[i][j-1][Y]);
    ...
    const int max_iter = (cgc1 > ATOL)? 1000:0;
    for (int itr = 0; itr < max_iter; itr++) {    
      ...
      if( bnorm*RTOL >= rnorm || rnorm < ATOL ) break;
    ...
    for (int p = 0; p < NP; p++){
      ...
      fp[p][X] = recvbuf[0] + mp[p]*F_EX[X];
      fp[p][Y] = recvbuf[1] + mp[p]*F_EX[Y];
      torp[p][0] = recvbuf[2];
    }
